...
const jwtConfig = {
  ...
  accessToken: {
    secret: process.env.JWT_SECRET || 'polox_super_secret_key_2025_change_in_production',
...
const sessionConfig = {
  ...
  cleanupInterval: parseInt(process.env.SESSION_CLEANUP_INTERVAL) || 60 * 60 * 1000, // 1 hora em ms
...
const tenantConfig = {
  ...
  plans: {
    ...
    'professional': {
      ...
      modules: ['dashboard', 'leads', 'clients', 'sales', 'products', 'finance', 'schedule', 'tickets', 'gamification'],
      features: ['advanced_reporting', 'email_notifications', 'sms_notifications', 'api_access']
    },
    'enterprise': {
...
const validateJWTConfig = () => {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (jwtConfig.accessToken.secret.includes('polox_super_secret_key')) {
      warnings.push('⚠️  Usando JWT_SECRET padrão em produção! Altere imediatamente.');
    }
    
    if (bcryptConfig.rounds < 12) {
      warnings.push('⚠️  BCRYPT_ROUNDS muito baixo para produção. Recomendado: 12+');
    }
  }
...
const getEnvironmentConfig = () => {
  ...
  return {
    ...
    allowInsecurePasswords: !isProduction && process.env.ALLOW_WEAK_PASSWORDS === 'true'
  };
};
