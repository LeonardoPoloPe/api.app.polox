...
// Storage S3 (AWS) - somente se configurado
let s3Storage = null;
if (process.env.AWS_S3_BUCKET) {
  try {
    s3Storage = require("multer-s3")({
    ...
    console.warn("⚠️  S3 Storage não configurado, usando armazenamento local");
  }
}
...
const fileFilter = (req, file, cb) => {
  ...
  if (!uploadConfig.allowedMimeTypes.includes(file.mimetype)) {
    return cb(
      new ApiError(400, `Tipo de arquivo não permitido: ${file.mimetype}`),
      false
    );
  }
  ...
  if (!uploadConfig.allowedExtensions.includes(extension)) {
    return cb(
      new ApiError(400, `Extensão de arquivo não permitida: .${extension}`),
      false
    );
  }
...
const upload = multer({
  storage: uploadConfig.useS3 && s3Storage ? s3Storage : localStorage,
