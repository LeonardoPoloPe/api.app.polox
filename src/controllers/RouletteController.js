...
class RouletteController {
    ...
    static async getRoulette(req, res, next) {
        try {
            ...
            if (!roulette) {
                return res.status(404).json({ success: false, message: 'Roleta não encontrada.' });
            }
    ...
    static async updateRoulette(req, res, next) {
        try {
            ...
            const prizeSchema = Joi.object({
                ...
                colorCode: Joi.string().regex(/^#[0-9A-F]{6}$/i).required(),
                prizeValue: Joi.number().min(0).default(0),
                prizeType: Joi.string().valid('discount_percent', 'cashback_value', 'physical_item', 'xp', 'coin').default('discount_percent'),
                resendLinkUrl: Joi.string().uri().allow(null, '').optional(),
                redirectionType: Joi.string().valid('url', 'whatsapp', 'in_app').default('url'),
            ...
            const updatedRoulette = await RouletteModel.updateRoulette(id, companyId, rouletteData, prizes);

            // Log de auditoria
            await AuditLogModel.logUpdate(
                'roulette', id, rouletteData.rouletteName, updatedRoulette, userId, companyId
            );
...
// ...existing code...
    ...
    static async createRoulette(req, res, next) {
        try {
            ...
            console.log('📥 Recebendo requisição POST /api/v1/roulette', { body: req.body });
            ...
            const { rouletteData, prizes } = await schema.validateAsync(req.body);
            console.log('✅ Schema validado:', { rouletteData, prizesCount: prizes.length });
            
            // 2. Lógica de Criação (Chamada direta ao Model com transação atômica)
            const normalizedData = {
                companyId: companyId,
                name: rouletteData.rouletteName,
            ...
            const newRoulette = await RouletteModel.createRoulette(normalizedData, prizes); 
            console.log('✅ Roleta criada:', newRoulette);

            // 3. Log de Auditoria
            console.log('📝 Criando log de auditoria...');
            await AuditLog.logCreate(
                'roulette', newRoulette.id, newRoulette.roulette_name, newRoulette, userId, companyId
            ); 
    ...
    static async generateRouletteLink(req, res, next) {
        try {
            ...
            const roulette = await RouletteModel.findRouletteById(rouletteId, companyId);
            ...
            await AuditLog.create({
                ...
                entity_id: rouletteId,
                entity_name: roulette.roulette_name,
                description: `Link da roleta enviado${contactId ? ' para o contato ID ' + contactId : ' avulso'}`,
                category: 'gamification',
                severity: 'info',
                metadata: { token: uniqueToken, contact_id: contactId }
            }, companyId);
    ...
    static async publicSpinRoulette(req, res, next) {
        try {
            ...
            const { rouletteId, contactId, companyId, vendorUserId } = await this._resolveToken(token);
            let finalContactId = contactId;
            
            // 2. Tratamento de Leads Avulsos (Chamada direta ao Model)
            if (!finalContactId) {
                if (!contactData) {
                    throw new ValidationError('Dados de contato (nome e email/telefone) são obrigatórios para leads avulsos.');
                }
                const validatedContactData = await contactDataSchema.validateAsync(contactData);
                
                // Chamada direta ao Model para find/create
                const lead = await ContactModel.findOrCreateLead(companyId, validatedContactData);
            ...
            const spinsCount = await RouletteModel.getSpinsCountForContact(rouletteId, finalContactId);
            ...
            const prizeWon = this._performWeightedDraw(prizes);
            ...
            const newSpin = await RouletteModel.recordSpin({
    ...
    static async publicClaimPrize(req, res, next) {
        try {
            ...
            const prizeArray = await RouletteModel.getPrizesByRouletteId(spinHistory.roulette_id);
            ...
            const redirectionUrl = prize.resend_link_url || `${BASE_URL}resgate-sucesso`;
            ...
            await AuditLog.create({
                ...
                entity_id: spinHistory.prize_id,
                entity_name: prize.prize_description,
                description: `Prêmio resgatado (Código: ${redeemCode})`,
                category: 'gamification_conversion',
                severity: 'info',
                metadata: { contact_id: spinHistory.contact_id, claimed_method: claimedMethod }
            ...
            if (error instanceof ApiError || error instanceof NotFoundError) {
                return res.status(error.statusCode).send(`Erro ao resgatar: ${error.message}`);
            }
            next(error); 
        }
    }
