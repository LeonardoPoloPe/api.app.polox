  ...
  for (let i = 0; i < iterations; i++) {
    try {
      ...
      if ((i + 1) % 10 === 0 || i === iterations - 1) {
        ...
        process.stdout.write(`\r   Progresso: ${progress}% (${i + 1}/${iterations})`);
      }
    } catch (error) {
      metrics.addError();
      if (error.response) {
        console.error(`\n   ⚠️  Erro ${error.response.status}: ${error.response.data?.message || 'Unknown'}`);
      } else {
...
function printReport(allMetrics) {
  ...
  console.log('┌─────────────────────────────────────────────────────────────────────────┐');
  console.log('│ Endpoint                        │  Avg  │  Min  │  Max  │  P95  │  P99  │');
  console.log('├─────────────────────────────────────────────────────────────────────────┤');
  ...
  for (const metrics of allMetrics) {
    ...
    const emoji = parseFloat(stats.avg) < 100 ? '🟢' : parseFloat(stats.avg) < 500 ? '🟡' : '🔴';
    console.log(`│ ${emoji} ${name}│ ${avg} │ ${min} │ ${max} │ ${p95} │ ${p99} │`);
    
    if (stats.errors > 0) {
      console.log(`│    ⚠️  Erros: ${stats.errors}/${stats.total}                                           │`);
    }
  }
  ...
  console.log(`   - Requisições bem-sucedidas: ${totalRequests} (${((totalRequests / (totalRequests + totalErrors)) * 100).toFixed(1)}%)`);
