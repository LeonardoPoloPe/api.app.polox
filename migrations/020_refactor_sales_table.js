...
async function up(client) {
  console.log('🔄 Iniciando Migration 020: Refatorar tabela sales - tags e notes...');
  ...
  try {
    ...
    await client.query(`
      ...
      console.log(`   Encontradas ${salesWithTags.rows.length} vendas com tags para migrar`);
      ...
      if (notesCount > 0) {
        console.log(`   ⚠️  ATENÇÃO: ${notesCount} vendas possuem notes que serão removidos`);
        console.log(`   💡 Se necessário, crie uma tabela sale_notes antes de executar esta migration`);
      }
...
async function down(client) {
  ...
  try {
    ...
    await client.query(`
      ...
      console.log(`   ✅ Tags migradas de volta para ${salesWithPivotTags.rows.length} vendas`);
